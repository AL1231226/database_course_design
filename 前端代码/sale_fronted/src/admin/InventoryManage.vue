<template>
  <div class="inventory">
    <div class="header-bar">
      <h2>库存管理</h2>
      <button class="btn btn-primary" @click="showForm = true; form = {}">+ 新增商品</button>
    </div>

    <table class="table">
      <thead><tr>
        <th>商品编号</th><th>分类编码</th><th>商品说明</th><th>单价</th><th>库存</th><th>生产厂家</th><th>操作</th>
      </tr></thead>
      <tbody>
    ...
    <div class="modal" v-if="showForm">
      <div class="modal-content">
        ...
        <div class="form">
          <div class="form-item"><label>商品编号</label><input v-model="form.productId" :disabled="!!form.productId"></div>
          ...
          <div class="form-item"><label>生产厂家</label>
            <select v-model="form.manufacturerCode">
              <option v-for="m in manufacturers" :key="m.manufacturerCode" :value="m.manufacturerCode">{{ m.manufacturerName }}</option>
            </select>
          </div>
...
export default {
  ...
  data() { return { products: [], manufacturers: [], showForm: false, form: {} }; },
  ...
  methods: {
    ...
    edit(p) { this.form = { ...p }; this.showForm = true; },
    async handleSave() {
...
.modal-content { background:#fff; border-radius:8px; padding:24px; min-width:450px; position:relative; }
